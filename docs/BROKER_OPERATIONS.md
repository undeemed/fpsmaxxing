...
The gateway does not connect to it yet - it still opens an in-process control plane of its own - so the broker path is exercised by the `BrokerClient` end-to-end tests in `crates/ipc` rather than by an MCP client.
...
In particular the broker deliberately does not read `FPSMAXXING_JOURNAL_PATH`, which belongs to the gateway and the CLI.

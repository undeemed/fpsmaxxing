...
Binding that socket is also what keeps single-owner-per-knob true across processes: an existing socket file is connect-probed, and a path something still answers on is refused rather than unlinked, so a second broker cannot take the endpoint over and drive the same knobs through its own ownership ledger.
Only a socket that refuses the probe is replaced, which is the stale file a crashed broker leaves behind, and the transport unlinks on drop only the entry it bound itself - matched by device and inode - so an exiting instance can never strand a live successor.
A supervisor-level single-instance unit can layer on top of that guard later; it is not a substitute for it, because the socket path is the resource two brokers actually contend for.

...
Where an entry states a class rather than a single item it does not enumerate that class: membership is derivable by sweeping the checked-in schemas for constraining keywords and applying the entry's own criterion to each carrier the sweep turns up.

...
The recorded samples themselves are not re-derived, and the hypothesis is held to its length rather than its content, so detecting a rewrite of the measurements together with the verdict they imply - or a rewrite of the hypothesis text within its bounds - needs a signed or hash-chained journal, which the alpha does not have.

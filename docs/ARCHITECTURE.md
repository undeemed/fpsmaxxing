...
The directory that directly holds the socket or the journal is held higher still - no other user may write it even when it is sticky - because sticky only stops another user renaming or removing the broker's entry, not creating that entry first in a shared directory like `/tmp` and keeping ownership of the file the broker then writes every `apply-intent` record into.
Above that directory, creating an entry is not the threat and swapping a directory is, which is exactly what sticky prevents, so the exemption stands where it is sound.

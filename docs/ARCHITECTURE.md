...
What keeps single-owner-per-knob true across processes is an exclusive advisory lock rather than the bind: the broker locks a `0600` file beside its journal before the journal is opened and before the socket is bound, so a second broker refuses to start instead of driving the same knobs through an ownership ledger of its own, and it refuses before it has touched either.
...
The lock scopes an instance to its journal, which is the durable state two brokers would actually corrupt; a supervisor-level single-instance unit can layer on top of it later.
